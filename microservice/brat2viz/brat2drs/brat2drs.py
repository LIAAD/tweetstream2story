...
def file_parser(filecontent):
    ...
    for x in filecontent:
        ...
        ann = {}
        if x[0] == 'E':
            #ann_pattern = re.compile("E[0-9]+ \s+ EVENT:T[0-9]+") # E1 EVENT:T20
            ann = {'ann_type': 'Event', 'tag_id': tmp[0], 'tag': tmp[1].split(
                ':')[0], 'tag_ref': tmp[1].split(':')[1]}
        elif x[0] == 'A':
            #ann_pattern = re.compile("A[0-9]+\s+\S+\s+T[0-9]+\s+\S+") # A1 Lexical_Head T1 Noun
            ann = {'ann_type': 'Attribute',
                   'tag_id': tmp[0], 'attr_type': tmp[1], 'tag_ref': tmp[2], 'value': ' '.join(tmp[3:])}
        elif x[0] == 'T':
            # T1 Participant 33 41 Portugal e Espanha
            # T1 Participant 33 ; 41 Portugal e Espanha
            #ann_pattern = re.compile("T[0-9]+\s+[\S]+\s+[0-9]+\s+((;\s+[0-9]+)|([0-9]+))\s+.*") 
            ...
            # R1 SEMROLE_theme Arg1:T33 Arg2:E1
            #ann_pattern = re.compile("R[0-9]+\s+SEMROLE_\S+\s+Arg[0-9]+:T[0-9]+\s+Arg[0-9]+:E[0-9]+") # A1 Lexical_Head T1 Noun
...
def assign_variable(ann_dict):
    ...
    for x in ann_dict:
        if x['ann_type'] == 'Event':
            dexpr_list.append(
                ({'event_var': var[i], 'event_tag_id': x['tag_id'], 'tag_ref': x['tag_ref']},
                 dexpr('event(%s)' % var[i])))
            i += 1
            x['var'] = var[i]
            ...
            dexpr_list.append(
                ({'event_var': var[i], 'event_tag_id': x['tag_id'], 'event_str': x['value']},
                 dexpr('event(%s)' % var[i])))
            i += 1
            x['var'] = var[i]
        elif x['ann_type'] == 'TextBound' and x['attribute'] == 'EVENT':
...
def get_actors(ann_dict):
    ...
    for x in ann_dict:
        ...
        if x['ann_type'] == 'TextBound' and \
                (x['attribute'] == 'ACTOR' or x['attribute'] == 'Participant'):
...
def TT_relations(relations, ann_dict, dexpr_list):
    ...
    for x in ann_dict:
        ...
        if x['ann_type'] == 'Relation' and ('OBJ_REL' in x['rel_type'] or 'REF_REL' in x['rel_type']):
            relations.append(
                {'ref1': x.get('tag_ref1'), 'ref2': x.get('tag_ref2'), 'relation': x.get('rel_type').split('_')[2]})
...
def main(ann_dir=ANN_DIR):
    ...
    for f in files:
        ...
        output_file_name = f.split(os.path.sep)[1].replace('.ann', '_drs.txt')
