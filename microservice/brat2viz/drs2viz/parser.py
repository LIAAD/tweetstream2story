...
def resolve_redundancy(redundant_dict, actors_dict, relations_triplets):
    ...
    for actor_1, redund_list in redundant_dict.items():
        ...
        for actor_aux in redund_list:
            # Add redundant actor to original actor in actor_dict, if the words are distinct
            distinct_actors = list(map(lambda x: x.lstrip().lower(), actors_dict[actor_1].split(',')))
            if not (actors_dict[actor_aux].lower() in distinct_actors):
