...
def load():
    ...
    pipeline['pt'] = spacy.load('pt_core_news_sm')
    pipeline['en'] = spacy.load('en_core_web_sm')

    neuralcoref.add_to_pipe(pipeline['en'])
...
def extract_objectal_links(lang, text):
    """Extracts the co-references from each entity in the text.
    Groups the co-references by the entity they refer to and finds the character span of each co-reference.
    Args:
        lang (str): the language of the text to be annotated
        text (str): the text to be annotated
    Returns:
        list[list[tuple(int, int)]]: Each list has tuples of char spans from entities in the same coref cluster
    """
    if lang not in ['pt', 'en']:
        raise InvalidLanguage(lang)
    ...
    for cluster in clusters:
        # print(cluster.main) -> cluster representative (not used)
        corefs = cluster.mentions
        mentions_list = []
        for ref in corefs:
            mentions_list.append((ref.start_char, ref.end_char))
        clusters_index_list.append(mentions_list)
    
    return clusters_index_list
