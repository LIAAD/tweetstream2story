...
def load():
    ...
    # url_coref = 'https://storage.googleapis.com/allennlp-public-models/coref-spanbert-large-2020.02.27.tar.gz'
    # coref_path = os.path.join(current_path, "cache")

    # if not(os.path.exists(coref_path)):
    #     os.mkdir(coref_path)

    # coref_path = os.path.join(coref_path, "coref-spanbert-large-2020.02.27.tar.gz")

    # if not(os.path.exists(coref_path)):
    #     print("Downloading %s. It can take some time..." % coref_path)
    #     urllib.request.urlretrieve("https://storage.googleapis.com/allennlp-public-models/coref-spanbert-large-2020.02.27.tar.gz",\
    #             coref_path)
...
def _srl_pipeline(sentence_df, text, char_offset, verb_tags, event_threshold=3):
    ...
    # This check is necessary, otherwise a dataframe with 0 cols will be converted to a Series and throw an error
    if (len(sentence_df.columns) != 0):
