...
class Narrative:
    ...
    def extract_times(self, *tools):
        ...
        times = Annotator(tools).extract_times(self.lang, self.text,
                                               self.publication_time)  # annotations :: [(TimeStartOffset, TimeEndOffset, TimeType, TimeValue)]
        print("Done")
        for time in times:
    ...
    def extract_events(self, *tools):
        ...
        for event in events.itertuples():
            ...
            print("E" + str(self._event_id) + " " + event.actor)
            self._event_id += 1
    ...
    def extract_objectal_links(self, *tools):
        ...
        """
        print("Extracting objectal links...")
        ...
        for cluster in clusters:
            for i in range(0, len(cluster) - 1):
                ...
                self.obj_links['R' + str(self._rel_id)] = ObjectalLink(arg1,
                                                                       arg2)  # (Type (sameHead, partOf, ...), Arg1, Arg2)
                self._rel_id += 1
    ...
    def ISO_annotation(self):
        ...
        for actor_id in self.actors:
            ...
            r += ('A' + str(
                attribute_id) + '\t' + 'Individuation_Domain' + ' ' + actor_id + ' ' + actor.individuation + '\n')
            attribute_id += 1

            # A3 Actor_Type T1 Per
            r += ('A' + str(attribute_id) + '\t' + 'Participant_Type_Domain' + ' ' + actor_id + ' ' + actor.type + '\n')
            attribute_id += 1
        ...
        for time_id in self.times:
            ...
            r += ('A' + str(
                attribute_id) + '\t' + 'TemporalFunction' + ' ' + time_id + ' ' + time.temporal_function + '\n')
            attribute_id += 1
        ...
        for event_id in self.events:
            ...
            r += ('T' + str(self._id) + '\t' + 'Event' + ' ' + str(event.character_span[0]) + ' ' + str(
        ...
        for objectal_link_id in self.obj_links:
            ...
            r += (
                    objectal_link_id + '\t' + 'OBJ_REL_' + obj_link.type + ' ' + 'Arg1:' + obj_link.arg1 + ' ' + 'Arg2:' + obj_link.arg2 + '\n')

        for sem_link_id in self.sem_links:
            sem_link = self.sem_links[sem_link_id]

            # R35 SEMROLE_theme Arg1:E1 Arg2:T1
            r += (f"{sem_link_id}\tSEMROLE_{sem_link.type} Arg1:{sem_link.actor} Arg2:{sem_link.event}\n")
